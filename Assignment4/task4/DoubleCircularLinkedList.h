...
template <class T>
void DoubleCircularLinkedList<T>::reverseInGroups(int k)
{
    if (!isEmpty() && k > 0)
    {
        cout << "Original Order of Linked list" << endl;
        struct Node<T> *init = this->head;
        do {
            cout << init->data << ' ';
            init = init->next;
        } while (init != this->head);
        cout << "After reversing the nodes in " << k << " groups: " << endl;
    ...
    cout << "Invalid Operation! Linked List is either empty or value of K is <= 0" << endl;
    return ;
}