...
class DoubleCircularLinkedList: public LinkedList<T>
{
    public:
        ...
        void display();
        void insertSorted(T);
        bool checkPalindrome(); // check if the linkedlist is palindrome or not  
        void rotateByNnodes(int pos); // rotate doubly linked list by N nodes 
        ~DoubleCircularLinkedList()
        {
            while (!isEmpty())
                deleteFromHead();
...
template <class T>
bool DoubleCircularLinkedList<T>::checkPalindrome()
{
    if (isEmpty())
    {
        cout << "Invalid Operation on empty linked list!" << endl;
    ...
    while (!isEmpty())    
    {
        arr1[itr] = deleteFromHead();
        arr2[itr] = deleteFromTail();
        if (isEmpty())
            continue;
...
template <class T>
void DoubleCircularLinkedList<T>::rotateByNnodes(int pos) 
{
    if (isEmpty() || pos == 0)
        return;
    struct Node<T> *temp = this->head;
    ...
    Node<T> *current = (pos > 0) ? this->head : this->head->prev;
    pos = abs(pos);
    while (pos > 0) 
    {
